...
public class ImGroupMessageCanalProcessor extends BaseCanalBinlogEventProcessor<CanalGroupMessageModel> {
    private final ImMessageElasticService messageEsService;

    @Override
    protected void processInsertInternal(CanalBinLogResult<CanalGroupMessageModel> result) {
        afterSaveOrUpdateEsMessage(result, false);
    }

    @Override
    protected void processUpdateInternal(CanalBinLogResult<CanalGroupMessageModel> result) {
        afterSaveOrUpdateEsMessage(result, true);
    }

    private void afterSaveOrUpdateEsMessage(CanalBinLogResult<CanalGroupMessageModel> result, boolean update) {
        CanalGroupMessageModel model = result.getAfterData();
        if (model != null) {
